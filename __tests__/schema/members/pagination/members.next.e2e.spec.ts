...
describe('[E2E] [MembersResolver] ...', () => {
  ...
  describe('', () => {
    ...
    beforeAll(async () => {
      ...
      dialog = await chatsService.create(users[0].user.id, ChatTypeEnum.GROUP);
    ...
    describe('[Getting] [Pagination] [Next] ...', () => {
      // start [] [] [] [] [] ... end
      it('should return the first 5 members of the chat', async () => {
        const [user] = users;
        ...
        expect(msgs).toStrictEqual({
          edges: members.slice(0, first).map(m => ({
          ...
          pageInfo: {
            ...
            endCursor: CursorCoder.encode({
              [key]: members[members.length - 1].id
            }),
            hasNextPage: true,
      ...
      // start ... [] [] [] [] [] ... end
      it('should return the first 5 members after the second member of the chat', async () => {
        const [user] = users;
        ...
        const res = await request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {
              ...
              membersPagination: {
                first,
                after: CursorCoder.encode({ [key]: members[1].id })
              }
            }
          });
        ...
        expect(msgs).toStrictEqual({
          edges: members.slice(2, first + 2).map(m => ({
            cursor: CursorCoder.encode({ [key]: m.id }),
            node: {
              id: m.id,
              role: m.role.name,
      ...
      it('should return the last 5 members of the chat', async () => {
        ...
        const res = await request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {
              ...
              membersPagination: {
                first,
                after: CursorCoder.encode({ [key]: members[4].id })
              }
            }
          });
        ...
        expect(msgs).toStrictEqual({
          edges: members.slice(5, first + 5).map(m => ({
            cursor: CursorCoder.encode({ [key]: m.id }),
            node: {
              id: m.id,
              role: m.role.name,
      ...
      it('should return the last 5 members of the chat after the third of the end', async () => {
        ...
        const res = await request(app.getHttpServer())
          ...
          .send({
            ...
            variables: {
              ...
              membersPagination: {
                first,
                after: CursorCoder.encode({ [key]: members[7].id })
              }
            }
          });
        ...
        expect(msgs).toStrictEqual({
          edges: members.slice(8, first + 8).map(m => ({
            cursor: CursorCoder.encode({ [key]: m.id }),
            node: {
              id: m.id,
              role: m.role.name,
