...
import {
  InvitationStatusSeeder,
  InvitationStatusFactory
} from '@db/seeds/invitation-status.seeder';
import {
  InvitationTypeSeeder,
  InvitationTypeFactory
} from '@db/seeds/invitation-type.seeder';
...
describe('[E2E] [InvitationsResolver] ...', () => {
  ...
  beforeAll(async () => {
    ...
    const invitationStatusSeeder = new InvitationStatusSeeder(
      new InvitationStatusFactory()
    );
    ...
    const invitationTypeSeeder = new InvitationTypeSeeder(
      new InvitationTypeFactory()
    );
  ...
  afterEach(async () => {
    await connection.query(`TRUNCATE friends, invitations CASCADE`);
  });
    ...
    describe('[Sending] ...', () => {
      it('should send the friend invitation', async () => {
        ...
        const resSend = await request(app.getHttpServer())
          ...
          .send({
            query: `
                            mutation($payload: CreateInvitationDTO!) {
                                me {
                                    invitations {
    ...
    describe('[Getting] ...', () => {
      it('should return invitations', async () => {
        ...
        for (let i = 0; i < senderNumberInvitations; ++i)
          await app
            .get<InvitationsService>(InvitationsService)
            .create(sender.user.id, recipient.user.id, Type.INVITE_TO_FRIENDS);

        for (let i = 0; i < recipientNumberInvitations; ++i)
          await app
            .get<InvitationsService>(InvitationsService)
            .create(recipient.user.id, sender.user.id, Type.INVITE_TO_FRIENDS);
        ...
        expect(
          Array.isArray(resSender.body.data.me.invitations.fromMe)
        ).toBeTruthy();
        expect(resSender.body.data.me.invitations.fromMe).toHaveLength(
          senderNumberInvitations
        );
        ...
        expect(
          Array.isArray(resNewSender.body.data.me.invitations.forMe)
        ).toBeTruthy();
        expect(resNewSender.body.data.me.invitations.forMe).toHaveLength(
          recipientNumberInvitations
        );
        ...
        expect(
          Array.isArray(resRecipientSender.body.data.me.invitations.fromMe)
        ).toBeTruthy();
        expect(resRecipientSender.body.data.me.invitations.fromMe).toHaveLength(
          recipientNumberInvitations
        );
        ...
        expect(
          Array.isArray(resNewRecipient.body.data.me.invitations.forMe)
        ).toBeTruthy();
        expect(resNewRecipient.body.data.me.invitations.forMe).toHaveLength(
          senderNumberInvitations
        );
      });
