...
describe('[E2E] [AuthModule] ...', () => {
  ...
  describe('[Login] ...', () => {
    it('should authorize the user and return the access and refresh tokens', async () => {
      ...
      expect(data.auth).toHaveProperty('signIn');
      expect(data.auth.signIn).toHaveProperty('accessToken');
      expect(data.auth.signIn).toHaveProperty('refreshToken', jwtToken);
    });
    ...
    it('should provide access to profile information after successful authorization', async () => {
      ...
      const tokens = resLogin.body.data.auth.signIn;
  ...
  describe('[Registration] ...', () => {
    it('should provide access to profile information after successful registration', async () => {
      ...
      const tokens = resRegister.body.data.auth.signUp;
  ...
  describe('[Refresh Tokens] ...', () => {
    it('should refresh the tokens', async () => {
      ...
      const resMe = await request(app.getHttpServer())
        ...
        .set(
          'Authorization',
          `Bearer ${resRefresh.body.data.auth.refresh.accessToken}`
        )
        .send({
          query: `
                        query {
    ...
    it('should return 401 status when trying to refresh tokens with the expired refresh token', async () => {
      ...
      const firstTokens = resFirstLogin.body.data.auth.signIn;
      ...
      expect(
        resRefresh.body.errors[0].extensions.exception.status
      ).toStrictEqual(401);
    });
  });
  ...
  describe('[Logout] ...', () => {
    it('should logout user', async () => {
      ...
      const resLogout = await request(app.getHttpServer())
        ...
        .set(
          'Authorization',
          `Bearer ${resLogin.body.data.auth.signIn.accessToken}`
        )
        .set(
          'Cookie',
          `jwt-token=${resLogin.body.data.auth.signIn.refreshToken}`
        )
        .send({
          query: `
            mutation {
    ...
    it('should return 401 status when try to logging out the unauthorized user', async () => {
      ...
      expect(
        resLogout.body.errors[0].extensions.exception.status
      ).toStrictEqual(401);
    });
  });
});
