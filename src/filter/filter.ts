...
export class AndOperator<T> implements ILogicalOperator {
    ...
    public exec(query: FilterArg[] = [], qb: WhereExpression) {
    	for (const q of query) {
    		if (q.field) {
    			if (q.AND || q.OR) throw new Error(`The field and logical operator cannot be use at the same time`);
...
export class Filter<T> {
    ...
    public make(q: FilterArg) {
    	...
    	this.builder.andWhere(new Brackets(qb => {
    		...
    		if (q.AND) 
    			this.andOperator.exec(q.AND, qb);

    		if (q.OR) 
    			this.orOperator.exec(q.OR, qb);
    	}));
