        ...
        describe('[Getting] [Pagination] [Previous] ...', () => {
            ...
            it('should return the last 5 messages of the chat', async () => {
                ...
                expect(msgs).toStrictEqual({
                    edges: messages.slice(messages.length-last, messages.length).map(m => ({ 
            ...
            it('should return the last 5 messages after the second (8) message of the end', async () => {
                ...
                const res = await request(app.getHttpServer())
                    ...
                    .send({
                        ...
                        variables: {
                            ...
                            messagePagination: {
                                last,
                                before: CursorCoder.encode({ [key]: messages[messages.length-2].id })
                            }
                        }
                    });
                ...
                expect(msgs).toStrictEqual({
                    edges: messages.slice(messages.length-last-2, messages.length-2).map(m => ({ 
                        cursor: CursorCoder.encode({ [key]: m.id }), 
                        node: { 
                            id: m.id, 
                            text: m.text, 
