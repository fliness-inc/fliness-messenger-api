        ...
        describe('[Getting] [Pagination] [Previous] ...', () => {
            ...
            it('should return the last 5 users of the user', async () => {
                ...
                expect(res.body).toStrictEqual({
                    data: {
                        users: {
                            edges: sortedUsers.slice(sortedUsers.length-last, sortedUsers.length).map(({ user }) => ({ 
            ...
            it('should return the last 5 users after the second (8) user of the end', async () => {
                ...
                expect(res.body).toStrictEqual({
                    data: {
                        users: {
                            edges: sortedUsers.slice(sortedUsers.length-last-2, sortedUsers.length-2).map(({ user }) => ({ 
                                cursor: CursorCoder.encode({ [key]: user.id }), 
                                node: { 
                                    id: user.id, 
                                    name: user.name,
            ...
            it('should return the last 5 users', async () => {
                ...
                expect(res.body).toStrictEqual({
                    data: {
                        users: {
                            edges: sortedUsers.slice(0, sortedUsers.length-5).map(({ user }) => ({ 
                                cursor: CursorCoder.encode({ [key]: user.id }), 
                                node: { 
                                    id: user.id, 
                                    name: user.name,
            ...
            it('should return the last 5 users of the user after the third of the end', async () => {
                ...
                expect(res.body).toStrictEqual({
                    data: {
                        users: {
                            edges: sortedUsers.slice(0, sortedUsers.length-last-3).map(({ user }) => ({ 
                                cursor: CursorCoder.encode({ [key]: user.id }), 
                                node: { 
                                    id: user.id, 
                                    name: user.name,
